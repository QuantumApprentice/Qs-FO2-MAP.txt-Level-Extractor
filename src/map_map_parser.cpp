...
enum flags
{
    ...
    MAP_HAS_ELEV_0  = 0x2,  // map has an elevation at level 0.
    MAP_HAS_ELEV_1  = 0x4,  // map has an elevation at level 1.
    MAP_HAS_ELEV_2  = 0x8   // map has an elevation at level 2.
};
...
map_header parse_header(uint8_t* map_file)
{
    ...
    h.global_var_cnt = B_Endian::read_i32(&map_file[48]);
...
void parse_map_map(map_lvls* map)
{
    ...
    // when a level is marked that means there's NO level information
    if (!(h.map_flags & MAP_HAS_ELEV_0)) {
        map->level[0] = "Level 0";
    }
    if (!(h.map_flags & MAP_HAS_ELEV_1)) {
        map->level[1] = "Level 1";
    }
    if (!(h.map_flags & MAP_HAS_ELEV_2)) {
        map->level[2] = "Level 2";
    }
